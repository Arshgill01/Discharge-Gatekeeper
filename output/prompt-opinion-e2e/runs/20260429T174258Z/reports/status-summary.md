...
| Run folder | `output/prompt-opinion-e2e/runs/20260429T174258Z` |

## Local automated checks
| Check | Status | Duration (ms) | Log |
| --- | --- | --- | --- |
| Install dependencies (Discharge Gatekeeper MCP) | GREEN | 5800 | `logs/00-npm-ci-typescript.log` |
| Install dependencies (Clinical Intelligence MCP) | GREEN | 2428 | `logs/00-npm-ci-clinical-intelligence.log` |
| Install dependencies (external A2A orchestrator) | GREEN | 2613 | `logs/00-npm-ci-external-a2a.log` |
| Full system validation | RED | 51255 | `logs/01-run-full-system-validation.log` |
| Boot two MCP runtimes | GREEN | 1575 | `logs/02-start-two-mcp-local.log` |
| Boot external A2A runtime | GREEN | 2510 | `logs/03-start-a2a-local.log` |
| Two-MCP readiness check | GREEN | 484 | `logs/04-check-two-mcp-readiness.log` |
| A2A readiness check | GREEN | 483 | `logs/05-check-a2a-readiness.log` |
| Prompt Opinion rehearsal smoke | GREEN | 2212 | `logs/06-smoke-prompt-opinion-rehearsal.log` |
| Boot two MCP runtimes for browser proof | GREEN | 1343 | `logs/07-start-two-mcp-browser-proof.log` |
| Boot external A2A runtime for browser proof | GREEN | 1351 | `logs/08-start-a2a-browser-proof.log` |
| Authenticated Prompt Opinion browser proof | RED | 37484 | `logs/09-prompt-opinion-browser-proof.log` |
...
| Prompt Opinion Direct-MCP fallback workspace lane | RED | Authenticated browser blocker: workspace_ui_unreachable. |
