  ...
  return new Promise((resolve, reject) => {
    ...
    server.listen(0, "127.0.0.1", () => {
      ...
      if (!address || typeof address === "string") {
        server.close();
        reject(new Error("Failed to reserve an open port for runtime smoke test."));
        return;
      }
...
const waitForHealthPayload = async (
  ...
  while (Date.now() < deadline) {
    if (serverProcess.exitCode !== null) {
      throw new Error(`MCP runtime exited before health check became ready (exit=${serverProcess.exitCode}).`);
    }
...
const main = async (): Promise<void> => {
  ...
  console.log("SMOKE PASS: runtime boot and tool registration");
};
