...
export class McpToolInvoker {
  ...
  async invokeDeterministicReadiness(input: A2ATaskInput): Promise<DeterministicResponse> {
    const scenarioId = input.patient_context?.scenario_id || this.config.defaultStructuredScenarioId;

    const result = await this.withClient(this.config.dischargeGatekeeperMcpUrl, async (client) => {
    ...
    const parsed = deterministicResponseSchema.safeParse(extractJsonToolResponse(result));
    if (!parsed.success) {
  ...
  async invokeHiddenRisk(
    ...
    input: A2ATaskInput,
  ): Promise<HiddenRiskResponse> {
    const narrative = input.patient_context?.narrative_evidence_bundle || [];
    const result = await this.withClient(this.config.clinicalIntelligenceMcpUrl, async (client) => {
      return client.callTool({
    ...
    const parsed = hiddenRiskResponseSchema.safeParse(extractJsonToolResponse(result));
    if (!parsed.success) {
