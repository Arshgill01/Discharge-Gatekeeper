...
const run = async (): Promise<void> => {
  ...
  try {
    ...
    assert.equal(cardPayload.endpoints.rpc, `http://127.0.0.1:${port}/rpc`);
    assert.equal(cardPayload.endpoints.message_send, `http://127.0.0.1:${port}/message:send`);
    assert.equal(cardPayload.endpoints.message_send_v1, `http://127.0.0.1:${port}/v1/message:send`);
