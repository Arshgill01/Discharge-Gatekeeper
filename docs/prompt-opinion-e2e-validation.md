...
Workspace credentials were provided and successfully used.
...
## Open risks
1. **A2A Registration Schema Strictness**: Prompt Opinion backend throws `422 Unprocessable Entity` when attempting to register the External A2A Agent due to `supportedInterfaces` enum mismatches. As defined in the Phase 0 Failure Plan, the demo must proceed using the direct-MCP fallback path (which we verified successfully via SSE).
